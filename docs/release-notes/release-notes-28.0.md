BitcoinDecentral Core version 28.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-28.0/>
...
Running BitcoinDecentral Core binaries on macOS requires self signing.
```
cd /path/to/bitcoindecentral-28.0/bin
xattr -d com.apple.quarantine bitcoindecentral-cli bitcoindecentral-qt bitcoindecentral-tx bitcoindecentral-util bitcoindecentral-wallet bitcoindecentrald test_bitcoin
codesign -s - bitcoindecentral-cli bitcoindecentral-qt bitcoindecentral-tx bitcoindecentral-util bitcoindecentral-wallet bitcoindecentrald test_bitcoin
```
...
Support for Testnet4 as specified in [BIP94](https://github.com/bitcoindecentral/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\BitcoinDecentral`
to `C:\Users\Username\AppData\Local\BitcoinDecentral`. BitcoinDecentral Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
- Previously if BitcoinDecentral Core was listening for P2P connections, either using
...
- BitcoinDecentral Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/bitcoindecentral/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running BitcoinDecentral Core with the configuration
...
are now required to compile BitcoinDecentral Core. (#29091, #30263)

- The minimum required glibc to run BitcoinDecentral Core is now
...
Previous releases of BitcoinDecentral Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
