BitcoinDecentral Core version 0.13.1 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming BitcoinDecentral wallet
  software more difficult and which seriously complicates the design of smart
  contracts for BitcoinDecentral.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each BitcoinDecentral block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** BitcoinDecentral addresses (both P2PKH addresses
  ...
  believed to be provided by BitcoinDecentral's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  BitcoinDecentral paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of BitcoinDecentral's current ledger (called the UTXO set),
  ...
  note that BitcoinDecentral Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  BitcoinDecentral users to individually opt-in to almost any change in the BitcoinDecentral
  Script language when those users receive new transactions.  Features
  currently being researched by BitcoinDecentral Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other BitcoinDecentral community members are studying several other improvements
...
[Segwit FAQ]: https://bitcoindecentral.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://bitcoindecentral.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/bitcoindecentral/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/bitcoindecentral/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/bitcoindecentral/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/bitcoindecentral/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://bitcoindecentral.org/en/2016/06/08/version-bits-miners-faq/
...
Since BitcoinDecentral Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/bitcoindecentral/bips/blob/master/bip-0147.mediawiki
...
- #8829 `2a8bca4` Add bitcoindecentral-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` bitcoindecentral-util-test.py should fail if the output file is empty (jnewbery)
...
- #8683 `8a7d7ff` Fix incorrect file name bitcoindecentral.qrc  (bitcoinsSG)
...
- #8817 `bcf3806` update bitcoindecentral-tx to output witness data (jnewbery)
