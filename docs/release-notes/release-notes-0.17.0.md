BitcoinDecentral Core version 0.17.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.17.0/>
...
BitcoinDecentral Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bitcoindecentral.conf` file, not inside included files or from
  ...
  as bitcoindecentral.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bitcoindecentral.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=bitcoindecentral
    test.uacomment=bitcoindecentral-testnet
...
be used in V0.17 by starting bitcoindecentrald with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed BitcoinDecentral Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoindecentral/bips/blob/master/bip-0174.mediawiki) is an interchange format for BitcoinDecentral transactions that are not fully signed
...
Overall, the construction of a fully signed BitcoinDecentral transaction goes through the
following steps:
...
- An **Extractor** produces a valid BitcoinDecentral transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BTCD. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When bitcoindecentral is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If bitcoindecentral is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart bitcoindecentrald with
...
- When running bitcoindecentrald with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If bitcoindecentrald
...
- #11962 `0a01843` add seed.bitcoindecentral.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in BTCD (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI bitcoindecentral:// (krab)
...
- #13604 `75bea05` Add depends 32-bit arm support for bitcoindecentral-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bitcoindecentral-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to bitcoindecentral.conf (MarcoFalke)
...
- #13687 `9d26b69` travis: Check that ~/.bitcoindecentral is never created (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in bitcoindecentrald systemd service file (Flowdalic)
- #12652 `55f490a` bitcoindecentral-cli: Provide a better error message when bitcoindecentrald is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to bitcoindecentrald in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for bitcoindecentral address in translations (kaplanmaxe)
...
- #13603 `453ae5e` bitcoindecentral-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bitcoindecentral-cli --version (Empact)
...
- #12877 `2b54155` Use bitcoindecentrald in Tor documentation (knoxcard)
...
- #13346 `60f0358` update bitcoindecentral-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust bitcoindecentral.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed bitcoindecentral-cli -help output for help2man (hebasto)
