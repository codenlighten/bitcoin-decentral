BitcoinDecentral Core version 0.13.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.13.0/>
...
updates will be released anymore. Without security updates, using a bitcoindecentral
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of BitcoinDecentral Core
randomly crashing on Windows XP. It is [not clear](https://github.com/bitcoindecentral/bitcoindecentral/issues/7681#issuecomment-217439891)
...
- Adding `dbcache=100` in bitcoindecentral.conf
...
bitcoindecentral-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitcoindecentral-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The BitcoinDecentral Core code base has
...
- `bitcoindecentral-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bitcoindecentral-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
compatible peers.
[BIP 152](https://github.com/bitcoindecentral/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of BitcoinDecentral Core.

[Pull request](https://github.com/bitcoindecentral/bitcoindecentral/pull/8035/files), [BIP 32](https://github.com/bitcoindecentral/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/bitcoindecentral/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/bitcoindecentral/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/bitcoindecentral/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/bitcoindecentral/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of BitcoinDecentral Core is expected that
...
Witness)] (https://github.com/bitcoindecentral/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/bitcoindecentral/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the BitcoinDecentral-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/bitcoindecentral/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/bitcoindecentral/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/bitcoindecentral/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/bitcoindecentral/bitcoindecentral/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for bitcoindecentral-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use BTCD/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to bitcoindecentral-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add bitcoindecentral address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: bitcoindecentral-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from bitcoindecentrald (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the BitcoinDecentral-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for bitcoindecentrald procs (MarcoFalke)
...
- #8164 `0f24eaf` [BitcoinDecentral-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of bitcoindecentrald startup (laanwj)
...
- #7975 `fc23fee` Update bitcoindecentral-core GitHub links (MarcoFalke)
