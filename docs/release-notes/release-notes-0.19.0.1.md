BitcoinDecentral Core version 0.19.0.1 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.19.0.1/>
...
BitcoinDecentral Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use BitcoinDecentral Core on unsupported systems.
...
to open the BitcoinDecentral Core .dmg. This is due to new signing requirements
imposed by Apple, which the BitcoinDecentral Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running BitcoinDecentral Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of BitcoinDecentral
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitcoindecentrald -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile BitcoinDecentral Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in BitcoinDecentral Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of BitcoinDecentral Core. (#14054)

- To eliminate well-known denial-of-service vectors in BitcoinDecentral Core,
  ...
  This prevents BitcoinDecentral Core from sending the BIP111 NODE_BLOOM service
  ...
  of BitcoinDecentral Core and nodes that have manually enabled BIP37 support,
...
- By default, BitcoinDecentral Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `bitcoindecentral-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how BitcoinDecentral Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  BitcoinDecentral Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, BitcoinDecentral Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 BitcoinDecentral address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BTCD), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BTCD per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in BitcoinDecentral Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@BitcoinDecentral AEAD (jonasschnelli)
...
- #15639 bitcoindecentral-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BTCD/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bitcoindecentrald/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitcoindecentral-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitcoindecentrald build (NicolasDorier)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding BitcoinDecentral Core (luke-jr)
...
- #16929 follow-up to rpc: default maxfeerate value as BTCD/kB (jonatack)
...
- #16489 log: harmonize bitcoindecentrald logging (jonatack)
...
- #15513 add "sections" info to example bitcoindecentral.conf (fanquake)
...
- #16448 add note on precedence of options in bitcoindecentral.conf (fanquake)
...
- #16621 add default bitcoindecentral.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitcoindecentral-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitcoindecentral.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitcoindecentrald.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitcoindecentral/bitcoindecentral/).
