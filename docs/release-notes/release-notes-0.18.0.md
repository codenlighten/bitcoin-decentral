BitcoinDecentral Core version 0.18.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.18.0/>
...
`/Applications/BitcoinDecentral-Qt` (on Mac) or `bitcoindecentrald`/`bitcoindecentral-qt` (on
Linux).
...
recommended to use BitcoinDecentral Core on unsupported systems.

BitcoinDecentral Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, BitcoinDecentral Core does not yet change appearance when
...
`contrib/init/bitcoindecentrald.service`, it has been changed to use
`/var/lib/bitcoindecentrald` as the data directory instead of
`~bitcoindecentral/.bitcoindecentral`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitcoindecentrald` will
exist has enough space (check using `df -h /var/lib/bitcoindecentrald`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoindecentral.org/en/contact/) the BitcoinDecentral Core
...
The systemd init file (`contrib/init/bitcoindecentrald.service`) has been changed
to use `/var/lib/bitcoindecentrald` as the data directory instead of
`~bitcoindecentral/.bitcoindecentral`. This change makes BitcoinDecentral Core more consistent with
...
`datadir` option in `/etc/bitcoindecentral/bitcoindecentral.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitcoindecentral/bitcoindecentral.conf`.


Documentation
-------------

- A new short [document](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/bitcoindecentral-conf.md)
  about the `bitcoindecentral.conf` file describes how to use it to configure
  BitcoinDecentral Core.

- A new document introduces BitcoinDecentral Core's BIP174 [Partially-Signed
  BitcoinDecentral Transactions
  (PSBT)](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BitcoinDecentral-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BitcoinDecentral Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BitcoinDecentral Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BitcoinDecentral Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitcoindecentrald with the `-deprecatedrpc=generate`
...
- A new `bitcoindecentral-wallet` tool is now distributed alongside BitcoinDecentral
...
This section describes planned changes to BitcoinDecentral Core that may affect
other BitcoinDecentral software and services.

- Since version 0.16.0, BitcoinDecentral Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BitcoinDecentral Core 0.20 (expected about
  a year after 0.18), BitcoinDecentral Core will default to native segwit
  ...
  to bech32 addresses, and if the BitcoinDecentral Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BitcoinDecentral Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BitcoinDecentral Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BitcoinDecentral Core
...
- A sub-project of BitcoinDecentral Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BitcoinDecentral Core.  See their [project
  page](https://github.com/bitcoindecentral-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BitcoinDecentral Core's own implementation, although entropy
  gathered by BitcoinDecentral Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BitcoinDecentral Core a
...
- On macOS, BitcoinDecentral Core now opts out of application CPU throttling
...
- #13926 [Tools] bitcoindecentral-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitcoindecentrald (MarcoFalke)
...
- #14521 qt, docs: Fix `bitcoindecentral-qt -version` output formatting (hebasto)
...
- #15154 Configure: bitcoindecentral-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitcoindecentral-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BitcoinDecentral Core (sipsorcery)
...
- #13953 Fix deprecation in bitcoindecentral-util-test.py (isghe)
...
- #12775 Integration of property based testing into BitcoinDecentral Core (Christewart)
...
- #12246 Bugfix: Only run bitcoindecentral-tx tests when bitcoindecentral-tx is enabled (luke-jr)
...
- #14186 bitcoindecentral-cli: don't translate command line options (HashUnlimited)
...
- #12255 Update bitcoindecentral.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitcoindecentral-conf.md (hebasto)
...
- #15354 Add missing `bitcoindecentral-wallet` tool manpages (MarcoFalke)
...
- #15802 mention creating application support bitcoindecentral folder on OSX (JimmyMow)
