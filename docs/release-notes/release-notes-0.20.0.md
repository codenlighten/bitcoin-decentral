...
BitcoinDecentral Core version 0.20.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from BitcoinDecentral Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitcoinDecentral Core version 0.18.0.
...
- Testing or debugging of implementations of the BitcoinDecentral P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitcoinDecentral Core. BitcoinDecentral Core logs debug messages
...
- OpenSSL is no longer used by BitcoinDecentral Core.  (#17265)

- BIP70 support has been fully removed from BitcoinDecentral Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitcoindecentral/bitcoindecentral/issues/16599),
  [PR #16702](https://github.com/bitcoindecentral/bitcoindecentral/pull/16702), and the
  `bitcoindecentrald help` for more information.  This option is experimental and
...
- All custom settings configured when BitcoinDecentral Core starts are now
...
- The "Start BitcoinDecentral Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoindecentral.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BitcoinDecentral Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BitcoinDecentral Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoindecentral-core/HWI) for
  signing.  Future versions of BitcoinDecentral Core should support a GUI option
...
- BitcoinDecentral Core's automatically-generated source code documentation is
  now available at https://doxygen.bitcoindecentral.org.  (#17596)
...
- The `bitcoindecentral-cli` utility used with the `-getinfo` parameter now
...
- The `bitcoindecentral-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #18702 Fix ASLR for bitcoindecentral-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitcoindecentral schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitcoindecentral.conf (sdaftuar)
...
- #18166 ci: Run fuzz testing test cases (bitcoindecentral-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitcoindecentral-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitcoinDecentral Core files (hebasto)
...
- #17119 Fix broken bitcoindecentral-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitcoindecentral-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitcoindecentral.org (laanwj)
...
- #17648 Rename wallet-tool references to bitcoindecentral-wallet (hel-o)
...
- #18341 Replace remaining literal BTCD with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitcoindecentrald startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitcoindecentral-cli -getinfo (malevolent)
