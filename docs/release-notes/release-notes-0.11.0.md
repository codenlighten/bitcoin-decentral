BitcoinDecentral Core version 0.11.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BTCD/kB fee to be
...
For example, add the following to `bitcoindecentral.conf`:
...
related to the blockchain in the bitcoindecentral system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BitcoinDecentral Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that BitcoinDecentral Core will 
...
For an example script see [bitcoindecentral-submittx](https://github.com/laanwj/bitcoindecentral-submittx).
...
- #6269 `95aca44` gitian: Use the new bitcoindecentral-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add bitcoindecentral logo to about screen
...
- #5809 `46bfbe7` Add bitcoindecentral-cli man page
...
- #5849 `41259ca` contrib/init/bitcoindecentrald.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename BitcoinDecentral-Qt.app to BitcoinDecentral-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoindecentral-tx)
