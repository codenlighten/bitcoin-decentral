UNIX BUILD NOTES
====================
Some notes on how to build BitcoinDecentral Core in Unix.
...
memory available when compiling BitcoinDecentral Core. On systems with less, gcc can be
...
To build BitcoinDecentral Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/bitcoindecentral/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/bitcoindecentral/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, BitcoinDecentral Core can
