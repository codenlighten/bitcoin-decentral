# CJDNS support in BitcoinDecentral Core

It is possible to run BitcoinDecentral Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinDecentral Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinDecentral network and individual nodes.
...
before it will work with your BitcoinDecentral Core node. This is described in steps
...
## Run BitcoinDecentral Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinDecentral Core
...
When enabled, this option tells BitcoinDecentral Core that it is running in an
...
IPv6 local network. This helps BitcoinDecentral Core perform better address management:
...
CJDNS support was added to BitcoinDecentral Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoindecentral-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinDecentral Core

There are several ways to see your CJDNS address in BitcoinDecentral Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoindecentral-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoindecentral-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitcoindecentral-cli help getnodeaddresses` for details.
