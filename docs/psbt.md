# PSBT Howto for BitcoinDecentral Core

Since BitcoinDecentral Core 0.17, an RPC interface exists for Partially Signed BitcoinDecentral
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoindecentral/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinDecentral transactions that are not fully signed
...
#### Multisig with multiple BitcoinDecentral Core instances
...
BitcoinDecentral Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoindecentral-cli` is used.
...
Later, when *V* BTCD has been received on *Amulti*, and Bob and Carol want to
