BitcoinDecentral Core version 29.0 is now available from:

  <https://bitcoindecentral.org/bin/bitcoindecentral-core-29.0/>
...
BitcoinDecentral Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. BitcoinDecentral
...
previously BitcoinDecentral Core would listen for incoming Tor connections on `127.0.0.1:8334`.
...
127.0.0.1:5556`, or configure bitcoindecentrald with `-bind=127.0.0.1:8334=onion` to get the previous behavior.
(#31223)
...
- As a safety check, BitcoinDecentral core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. BitcoinDecentral Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
3. CMake variables may be used to configure the build system. See [Autotools to CMake Options Mapping](https://github.com/bitcoindecentral-core/bitcoindecentral-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with BitcoinDecentral Core. Therefore, the build system replaces it with `-O2`.
...
```
cmake -B build
cmake --build build --target bitcoindecentrald
cmake --install build --component bitcoindecentrald
```
