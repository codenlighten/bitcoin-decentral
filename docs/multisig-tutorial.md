...
Currently, it is possible to create a multisig wallet using BitcoinDecentral Core only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/bitcoindecentral/bitcoindecentral/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the bitcoindecentral node on the signet network.

```bash
./build/bin/bitcoindecentrald -signet -daemon
```

This tutorial also uses the default WPKH derivation path to get the xpubs and does not conform to [BIP 45](https://github.com/bitcoindecentral/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/bitcoindecentral/bips/blob/master/bip-0087.mediawiki).

At the time of writing, there is no way to extract a specific path from wallets in BitcoinDecentral Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/bitcoindecentral-cli -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, BitcoinDecentral Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in BitcoinDecentral Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
As previously mentioned, this step extracts the `m/84'/1'/0'` account instead of the path defined in [BIP 45](https://github.com/bitcoindecentral/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/bitcoindecentral/bips/blob/master/bip-0087.mediawiki), since there is no way to extract a specific path in BitcoinDecentral Core at the time of writing.
...
external_desc_sum=$(./build/bin/bitcoindecentral-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/bitcoindecentral-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
`external_desc` and `internal_desc` specify the output type (`wsh`, in this case) and the xpubs involved. They also use BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/bitcoindecentral/bitcoindecentral/issues/21199#issuecomment-780772418)].

Note that at least two descriptors are usually used, one for internal derivation paths and one for external ones. There are discussions about eliminating this redundancy, as can be seen in the issue [#17190](https://github.com/bitcoindecentral/bitcoindecentral/issues/17190).
...
Documentation for these and other parameters can be found by typing `./build/bin/bitcoindecentral-cli help importdescriptors`.
...
```bash
./build/bin/bitcoindecentral-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/bitcoindecentral-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/bitcoindecentral-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/bitcoindecentral-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/bitcoindecentral-cli -a $receiving_address
```
...
```bash
./build/bin/bitcoindecentral-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed BitcoinDecentral Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a BitcoinDecentral transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoindecentral/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid bitcoindecentral address.
...
```bash
balance=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/bitcoindecentral-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/bitcoindecentral-cli -signet decodepsbt $funded_psbt

./build/bin/bitcoindecentral-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/bitcoindecentral-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/bitcoindecentral-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/bitcoindecentral-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/bitcoindecentral-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/bitcoindecentral-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
