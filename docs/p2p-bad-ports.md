When BitcoinDecentral Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no BitcoinDecentral node is listening,
or one where a service is listening that is not related to the BitcoinDecentral network.
As a result, this service may occasionally get connection attempts from BitcoinDecentral
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by BitcoinDecentral Core,
trying to connect because it thinks there is a BitcoinDecentral node on that
...
Below is a list of "bad" ports which BitcoinDecentral Core avoids when choosing a peer to
...
[pull/23306](https://github.com/bitcoindecentral/bitcoindecentral/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/bitcoindecentral/bitcoindecentral/pull/23542)
