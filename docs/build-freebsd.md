...
This guide describes how to build bitcoindecentrald, command-line utilities, and GUI on FreeBSD.
...
Now that `git` and all the required dependencies are installed, let's clone the BitcoinDecentral Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `bitcoindecentrald` or `bitcoindecentral-qt`.
...
```
to: /path/to/bitcoindecentral/depends/x86_64-unknown-freebsd[release-number]
```
