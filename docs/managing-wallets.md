...
Since version 0.21, BitcoinDecentral Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `bitcoindecentral-cli help createwallet`.

```
$ bitcoindecentral-cli createwallet "wallet-01"
```
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.bitcoindecentral/wallets`                             |
| Windows          | `C:\Users\<user>\AppData\Local\BitcoinDecentral\wallets`             |
| macOS            | `/Users/<user>/Library/Application Support/BitcoinDecentral/wallets` |
...
```
$ bitcoindecentral-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ bitcoindecentral-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ bitcoindecentral-cli -named createwallet wallet_name="wallet-01" passphrase="passphrase"
```
...
```
$ bitcoindecentral-cli -rpcwallet="wallet-01" sendtoaddress "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx" 0.01
error code: -13
error message:
...
```
$ bitcoindecentral-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ bitcoindecentral-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
```
$ bitcoindecentral-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ bitcoindecentral-cli -rpcwallet="restored-wallet" getwalletinfo
```
...
Understanding wallet security is crucial for safely storing your BitcoinDecentral. A key aspect is the wallet passphrase, used for encryption. Let's explore its nuances, role, encryption process, and limitations.
