...
This guide describes how to build bitcoindecentrald, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
### 3. Clone BitcoinDecentral Repository
...
The BitcoinDecentral Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the BitcoinDecentral Core project provides presets, which are used in this guide.
...
To improve the build process performance, one might add the BitcoinDecentral repository directory to the Microsoft Defender Antivirus exclusions.
