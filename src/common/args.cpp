...
const char * const BITCOIN_CONF_FILENAME = "bitcoindecentral.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoindecentral-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BitcoinDecentral
    //   new: C:\Users\Username\AppData\Local\BitcoinDecentral
    // macOS: ~/Library/Application Support/BitcoinDecentral
    // Unix-like: ~/.bitcoindecentral
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinDecentral";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitcoinDecentral";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitcoinDecentral";
#else
    // Unix-like
    return pathRet / ".bitcoindecentral";
#endif
#endif
}
