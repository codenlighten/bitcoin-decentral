...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitcoindecentral-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitcoindecentral-tx tool is used for creating and modifying bitcoindecentral transactions.\n\n"
                "bitcoindecentral-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitcoindecentral transaction, or with \"-create [commands]\" to create a hex-encoded bitcoindecentral transaction.\n"
                "\n"
                "Usage: bitcoindecentral-tx [options] <hex-tx> [commands]\n"
                "or:    bitcoindecentral-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoindecentral transaction
