 ...
 */
static const char* BITCOIN_PID_FILENAME = "bitcoindecentrald.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    // UPnP support was dropped. We keep `-upnp` as a hidden arg to display a more user friendly error when set. TODO: remove (here and below) for 30.0. NOTE: removing this option may prevent the GUI from starting, see https://github.com/bitcoindecentral-core/gui/issues/843.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if bitcoindecentral is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoindecentral is started while in a temporary directory.\n",
