...
/** Call node.updateRwSetting() with BitcoinDecentral 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitcoinDecentral 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitcoindecentral/bitcoindecentral/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitcoindecentral 22.x will not try to read these.
...
//! Convert enabled/size values to bitcoindecentral -prune setting.
...
//! Get pruning enabled value to show in GUI from bitcoindecentral -prune setting.
...
//! Get pruning size value to show in GUI from bitcoindecentral -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::BTCD));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bitcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bitcoin_unit = BitcoinUnit::BTCD;
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitcoindecentral.conf value
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitcoindecentral/bitcoindecentral/pull/8273
    ...
    // (https://github.com/bitcoindecentral-core/gui/issues/567).
