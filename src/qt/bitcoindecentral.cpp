...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitcoindecentral.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoindecentral.qrc)
...
static const char* qt_argv = "bitcoindecentral-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitcoindecentral: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoindecentral);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitcoindecentral-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitcoindecentral-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitcoindecentral.conf, determine network, switch to network specific
    ...
    // bitcoindecentral: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
