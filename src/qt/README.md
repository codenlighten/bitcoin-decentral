This directory contains the source code for the BitcoinDecentral Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the BitcoinDecentral UI.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bitcoindecentral:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: BTCD / mBTC / etc. handling
* `callback.h`
...
4. Enter `bitcoindecentral-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitcoindecentral-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `bitcoindecentral-qt`
9. You're all set! Start developing, building, and debugging the BitcoinDecentral Core GUI
