...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinDecentral address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bitcoindecentral: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoindecentral"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinUnits::parse(BitcoinUnit::BTCD, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitcoindecentral:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnit::BTCD, info.amount, false, BitcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinConf()
{
    ...
    /* Open bitcoindecentral.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinDecentral.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinDecentral (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BitcoinDecentral (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinDecentral*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "bitcoindecentral.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitcoindecentral-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a bitcoindecentral.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=BitcoinDecentral\n";
        else
            optionFile << strprintf("Name=BitcoinDecentral (%s)\n", ChainTypeToString(chain));
