...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/bitcoindecentral/bitcoindecentral/pull/14884#discussion_r239585126).
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 BTCD) in its wallet. Using them is much more
...
with the bitcoindecentrald(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bitcoindecentrald.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the BitcoinDecentral Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a bitcoindecentrald node over the p2p interface.
