...
class AbandonConflictTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify that B and C's 10 BTCD outputs are available for spending again because AB1 is now conflicted
        assert_equal(alice.gettransaction(txAB1)["confirmations"], -1)
        newbalance = alice.getbalance()
        assert_equal(newbalance, balance + Decimal("20"))
        balance = newbalance

        # Invalidate the block with the double spend. B & C's 10 BTCD outputs should no longer be available
