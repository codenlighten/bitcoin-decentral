#!/usr/bin/env python3
# Copyright (c) 2017-present The BitcoinDecentral Core developers
...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'bitcoindecentral.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'[InitConfig] Data directory "{self.nodes[0].datadir_path}" contains a "bitcoindecentral.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bitcoindecentral.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'bitcoindecentral.conf', 'a', encoding='utf-8') as conf:
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a bitcoindecentral.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoindecentral.conf" file which is ignored, because a '
        ...
        # the same bitcoindecentral.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/bitcoindecentral.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when bitcoindecentral.conf in the default data directory sets another datadir '
                      'and it contains a different bitcoindecentral.conf file that would be ignored')
        ...
        # bitcoindecentral.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "bitcoindecentral.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitcoindecentral.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/bitcoindecentral.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "bitcoindecentral.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "bitcoindecentral.conf"
