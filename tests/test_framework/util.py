...
def assert_fee_amount(fee, tx_size, feerate_BTC_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s BTCD too low! (Should be %s BTCD)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s BTCD too high! (Should be %s BTCD)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting BTCD values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in BTCD given a feerate is BTCD/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  BTCD
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "bitcoindecentral.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "BitcoinDecentral"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/BitcoinDecentral"
        else:
            datadir = temp_dir / ".bitcoindecentral"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "bitcoindecentral.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "bitcoindecentral.conf")):
        with open(os.path.join(datadir, "bitcoindecentral.conf"), 'r', encoding='utf8') as f:
            for line in f:
