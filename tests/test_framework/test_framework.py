#!/usr/bin/env python3
# Copyright (c) 2014-present The BitcoinDecentral Core developers
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for a bitcoindecentral test script.

    Individual bitcoindecentral test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitcoindecentral-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bitcoindecentrald": ("bitcoindecentrald", "BITCOIND"),
            "bitcoindecentral-cli": ("bitcoincli", "BITCOINCLI"),
            "bitcoindecentral-util": ("bitcoinutil", "BITCOINUTIL"),
            "bitcoindecentral-wallet": ("bitcoinwallet", "BITCOINWALLET"),
        }
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoindecentrald', self.options.bitcoindecentrald) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bitcoindecentral-cli', self.options.bitcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitcoindecentrald test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitcoindecentrald test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bitcoindecentrald
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoindecentrald's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitcoindecentrald=self.options.bitcoindecentrald,
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoindecentral.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if bitcoindecentrald has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoindecentrald has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if bitcoindecentrald has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoindecentrald has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitcoindecentral-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitcoindecentral-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if bitcoindecentral-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("bitcoindecentral-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoindecentral-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitcoindecentral-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bitcoindecentral-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitcoindecentral-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether bitcoindecentral-util was compiled."""
