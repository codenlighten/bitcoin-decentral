#!/usr/bin/env python3
# Copyright (c) 2017-2022 The BitcoinDecentral Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Class for bitcoindecentrald node under test"""
...
BITCOIN_PID_FILENAME_DEFAULT = "bitcoindecentrald.pid"
...
class TestNode():
    """A class for representing a bitcoindecentrald node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bitcoindecentrald, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the bitcoindecentral.conf file.
        # This means that starting a bitcoindecentrald using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
    ...
    def __del__(self):
        # Ensure that we don't leave any bitcoindecentrald processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then bitcoindecentrald would bind P2P ports on
        ...
        # bitcoindecentral.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time bitcoindecentrald is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by bitcoindecentrald, and
        ...
        self.running = True
        self.log.debug("bitcoindecentrald started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the bitcoindecentrald process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'bitcoindecentrald exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; bitcoindecentrald is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to bitcoindecentrald after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; bitcoindecentrald is still starting
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into bitcoindecentrald")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to bitcoindecentrald
        expected_msg: regex that stderr should match when bitcoindecentrald fails

        Will throw if bitcoindecentrald starts without an error.
        Will throw if an expected_msg is provided and it does not match bitcoindecentrald's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'bitcoindecentrald exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'bitcoindecentrald should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to bitcoindecentral-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bitcoindecentral-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run bitcoindecentral-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running bitcoindecentral-cli {}".format(p_args[2:]))
