...
class HTTPBasicsTest(BitcoinTestFramework):
    ...
    def conf_setup(self):
        #Append rpcauth to bitcoindecentral.conf before initialization
        ...
        with open(self.nodes[0].datadir_path / "bitcoindecentral.conf", "a", encoding="utf8") as f:
            f.write(rpcauth + "\n")
            f.write(rpcauth2 + "\n")
            f.write(rpcauth3 + "\n")
        with open(self.nodes[1].datadir_path / "bitcoindecentral.conf", "a", encoding="utf8") as f:
            f.write("rpcuser={}\n".format(self.rpcuser))
    ...
    def run_test(self):
        ...
        assert_equal(401, call_with_auth(self.nodes[0], 'user1', 'bitcoindecentral').status)
