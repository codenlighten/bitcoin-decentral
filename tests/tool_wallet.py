...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your BitcoinDecentral, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `bitcoindecentral-wallet help` raises an error. Use `bitcoindecentral-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `bitcoindecentral-wallet -help` for valid methods.')
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/bitcoindecentral/bitcoindecentral/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of bitcoindecentral-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
        bad_ver_wallet_dump = self.nodes[0].datadir_path / "wallet-bad_ver2.dump"
        dump_data["BITCOIN_CORE_WALLET_DUMP"] = "2"
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of bitcoindecentral-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
    ...
    def test_dump_unclean_lsns(self):
        ...
        # Lastly kill bitcoindecentrald so that the LSNs don't get reset
